...
// Attenuation: Constant, linear, quadratic - light fading by distance
	...
	for (int i = 0; i < activeLights; ++i) {
		...
		if (lightType[i] == POINT) { 
			...
			if (spotDotCutoff > lightCutoff[i]) {
				...
				if (difIntensity > 0) {
					vec3 diffusePart = 
						difIntensity * lightDiffuse[i].xyz * materialDiffuse.xyz;
			...
			if (texture(shadowMap, shadowCoord.xy).x < shadowCoord.z - shadowCalcBias) {
				visibility = 0.5;
			}

			diffuseLuminosity += diffuseToAdd * visibility;
			specularLuminosity += specularToAdd * visibility;
		}
	} // End of for each active light loop
